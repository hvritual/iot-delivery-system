...
vi.mock("@/components/delivery-workspace", () => ({
  DeliveryWorkspace: ({
    ...
    <div data-testid="delivery-workspace">
      <span>{sessionName}</span>
      <button onClick={onAccount}>账号与管理</button>
      <button onClick={onLogout}>退出</button>
      {notice}
      {accountOpen ? accountContent : null}
    </div>
  ),
}));
...
describe("YU-28 local auth shell", () => {
  it("shows explicit local and OIDC login choices and establishes local session without OIDC configuration", async () => {
    ...
    expect(
      await screen.findByRole("form", { name: "本地成员登录" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "使用 OIDC 登录" }),
  ...
  it("shows verified current-member facts but does not claim administrator authority before the server decides", async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        if (path === "/auth/local/current") return Response.json(localMember);
        if (path === "/auth/local/admin/members")
    ...
    expect(
      screen.getByText(/提交后由服务端 durable authorization 决定/),
    ).toBeInTheDocument();
  });
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        if (path === "/auth/session")
          return Response.json({ authenticated: true, csrfToken });
        if (path === "/auth/local/current") return Response.json(localMember);
        if (path === "/auth/local/admin/project-role-bindings")
...
function requestPath(input: string | URL | Request): string {
  if (typeof input === "string")
    return new URL(input, "https://app.example").pathname;
