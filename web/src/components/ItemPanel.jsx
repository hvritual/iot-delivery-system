  ...
  return (
    <aside className="item-panel">
      ...
      <form className="item-details-form" onSubmit={saveDetails}>
        ...
        <label>IoT 绑定（每行：类型 | 引用 | 标签 | JSON 属性）<textarea onChange={(event) => setDetails((current) => ({ ...current, iotBindingsText: event.target.value }))} placeholder={'device | SN-001 | 测试机 | {"site":"lab-a"}\nfirmware | fw-2.8.0 | 固件版本 | {}\ncustomer | CUST-01 | 客户 | {}\nenvironment | staging | 预发布 | {}\nrollout_batch | gray-01 | 首批灰度 | {}'} rows="5" value={details.iotBindingsText} /></label>
        <label>研发证据关联（每行：类型 | 引用 | 标题 | URL | 状态 | 记录时间）<textarea onChange={(event) => setDetails((current) => ({ ...current, traceLinksText: event.target.value }))} placeholder={"pull_request | PR-12 | 灰度实现 | https://... | merged | 2026-09-05T08:09:10Z\nbuild | build-88 | 固件构建 | https://... | success | 2026-09-05T08:09:10Z\ntest | test-21 | 回归测试 | https://... | passed | 2026-09-05T08:09:10Z\ndefect | BUG-33 | 已知缺陷 | https://... | open | 2026-09-05T08:09:10Z\nrelease | rel-2.8.0 | 发布证据 | https://... | released | 2026-09-05T08:09:10Z"} rows="6" value={details.traceLinksText} /></label>
