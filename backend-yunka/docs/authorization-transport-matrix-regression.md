...
权限字典、生成 OperationPlan 与传输入口的登记口径为 **12/12 REST↔gRPC**；其中有 MCP 公共工具映射的是 **10/10 MCP**。允许结果通过同一 Operations/Executor/SQLite-Outbox 链执行；拒绝统一为 `unauthenticated` 或 `permission_denied`，拒绝路径不得提交业务对象或 Outbox。
...
`TestAuthorizationTransportMatrixHasTwelveRPCAndTenMCPPublicOperations` 从权限字典和生成 `operation-plans.json` 计算并断言 12 个 REST↔gRPC operation 与 10 个不重复 MCP 公共映射；它拒绝缺 REST/gRPC、重复 MCP 或计数漂移。`TestMCPRegistrationContainsExactlyTenDictionaryPublicToolsAndSevenExcludedExtensions` 通过 MCP `tools/list` 实际登记验证同一 10 个工具；项目列表、相似项、周任务、进度、排期、保存/列出视图这 7 个本地扩展明确不计入公共矩阵。
...
N/A 是架构限制，不以测试伪造生产认证通道。测试夹具可注入 Principal 来验证 post-auth authorization；这 **not a production network-authentication proof**，不连接真实 OIDC、数据库外部服务或 Vault。
...
- RED：`go test ./internal/bootstrap -run TestProductionAuthorizationMatrixAllowsRegisteredOperationClasses -count=1` 首次将夹具连接到 legacy service extension 时，`delivery.items.list` 被未登记扩展路径拒绝；GREEN 将矩阵夹具改为只装配生成 OperationPlan 的 Operations，测试随即通过。
...
- `go test . -run TestAuthorizationTransportMatrixHasTwelveRPCAndTenMCPPublicOperations -count=1`
- 最终门禁：`go test ./... -count=1`、`go vet ./...`、`git diff --check`。

## 已知边界和未做事项

- 本任务未新增 REST、gRPC 或 MCP 的生产认证通道；只在统一适配边界收口既有授权错误语义。
- 未进入 S0-04；未修改 `backend`、`third_party/yunka` 或生成文件；未 push、部署或连接真实 OIDC/数据库/Vault。
- 生产网络认证及外部身份提供方联调不在本回归的证据范围内。
