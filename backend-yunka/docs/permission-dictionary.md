...
`internal/localauth` remains a development-only compatibility mapper for legacy local API-key roles. The dictionary records four exact local effective-permission profiles and the gate compares them bidirectionally with `NewGrantResolver`. They are not one-to-one production role mappings: for example, `release-manager` composes create/update context permissions as well as release approval. All delivery operations are now canonical, so the legacy extension-alias list is empty.

This is an intentional tightening from the older generic item permissions, not a claim of complete behavior compatibility. These profiles are not production permissions or RoleBinding records, and the literal `local` scope must not be reused by production authorization.
