...
func TestApplyMigrationsSeedsExactAuthorizationDictionary(t *testing.T) {
	...
	if len(dictionary.Roles) != 6 || len(dictionary.Permissions) != 20 {
		t.Fatalf("authoritative permission dictionary has %d roles and %d permissions, want 6 and 20", len(dictionary.Roles), len(dictionary.Permissions))
	}
