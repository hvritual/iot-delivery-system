...
INSERT INTO service_accounts (id, organization_id, name) VALUES ('service-a', 'org-a', 'ci');`); err != nil {
	...
	for _, migrationID := range []string{MigrationID, ServiceCredentialMigrationID, AuthorizationMigrationID, ServiceGrantMigrationID, ProjectReadAuthorizationMigrationID} {
		var count int
	...
	var ledgerCount int
	if err := database.QueryRow(`SELECT COUNT(*) FROM iotd_schema_migrations`).Scan(&ledgerCount); err != nil || ledgerCount != 5 {
		t.Fatalf("repeat migration ledger count = %d error=%v, want 5", ledgerCount, err)
	}
}
