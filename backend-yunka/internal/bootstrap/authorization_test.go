...
func TestProductionAuthorizationUsesHumanResolverAndSharedRootGuard(t *testing.T) {
	...
	guard, ok := guards.ResolveGuard("delivery.projects.list")
	if !ok || guard == nil {
		t.Fatal("production shared operation guard is not configured")
	}
