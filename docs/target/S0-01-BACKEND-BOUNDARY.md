...
- **源码锚点：** `c3792c5b2c78ee82bd8832b1daa25d99ac653193`（S0-01-08 在此基线上实施并验证）
- **范围：** S0-01-02 架构决策及 S0-01-08 当前态收口；不迁移、切换或写入真实数据库/Vault。
...
- **虚线红边**表示旧后端或公开领域实现端口（CUR / boundary），不得当作 transport 写入许可。
- “正式合同”当前为 12 个 OperationPlan（10 写、2 读）：S0-01-03 覆盖 Project/Release/Sprint/Milestone 创建，S0-01-04 覆盖 WorkItem 创建、更新和评论，S0-01-05 覆盖 UpdateContext、AdvanceGate、Close。SaveView、相似度和排期读模型仍是显式本地扩展。
...
flowchart LR
  ...
  Seed[启动 seedExample\nbootstrap 内部 principal] --> Ops
  Service[公开 Service 方法\n内部领域实现端口] -. 仅 Operations/adapters 内部调用 .-> Repo

  classDef target fill:#eaf7ee,stroke:#227a3b,color:#111;
  classDef boundary fill:#fff4e5,stroke:#b85c00,color:#111;
  class Web,BFF,REST,GRPC,MCP,Jobs,Contract,Ops,Guard,UoW,Repo,Staging,Commit,Dispatcher,Projection,Notify,Seed target;
...
| --- | --- | --- | --- |
| CUR-REST-01～10 | 当前事实 | REST 注册写路由，handler 经 `Operations` 进入业务路径；S0-01-06 已收口关卡与关闭的方法守卫。 | [routes](../../backend-yunka/internal/httpapi/handler.go)；[actions](../../backend-yunka/internal/httpapi/handler.go) |
| CUR-GRPC（历史 S0-01-01 为 4 个） | 当前事实 | 当前 12 个 OperationPlan（10 写、2 读）由生成 adapter/RPC executor 以 `ExecuteTyped` 调用 executor；S0-01-05 已覆盖 UpdateContext、AdvanceGate、Close。 | [plans](../../backend-yunka/contracts/generated/operation-plans.json)；[RPC executor](../../backend-yunka/internal/delivery/transport/rpc/zz_yunka_management_operation_executor_gen.go) |
| CUR-MCP-01～10 | 当前事实 | MCP 写工具先建立本地已认证 principal，再调 `Operations`。 | [tools](../../backend-yunka/internal/mcpserver/server.go) |
| CUR-INT-01～05 | 当前事实 | bootstrap 装配 executor、SQLite transaction factory、Outbox dispatcher 和运行组件。 | [bootstrap assembly](../../backend-yunka/internal/bootstrap/application.go) |
| CUR-SVC-01～12 | 当前事实 / 潜在风险 | `Service` 仍导出为领域实现面，但 `Application.Service` 已删除；当前生产调用点受 AST allowlist 约束，仅 application adapter/Operations 内部可调用。历史 as-of 盘点不改写。 | [guard](../../backend-yunka/no_bypass_guard_test.go)；[catalog](../baseline/S0-01-WRITE-ENTRYPOINTS.json) |
| CUR-INT seed | 当前事实 / 已验证 | `seedExample` 默认关闭；只有显式 `development + example` 时，才以 bootstrap 信任边界内直接构造、`Authenticated=true` 的内部 principal（非人类/API key 登录）经 `Operations.List/Create/UpdateContext` 执行。样例写与 Outbox staging 在 SQLite Unit of Work 内提交，dispatcher 再异步投影。 | [bootstrap](../../backend-yunka/internal/bootstrap/application.go)；[S0-02-08 gate](S0-02-08-BOOTSTRAP-FAIL-CLOSED-LEAKAGE-GATE.md) |
...
| REST | 注册业务 HTTP 路由；`Operations` 作为 service 参数；S0-01-06 已修复并以结构回归锁定方法守卫。 | 仅将 HTTP 请求适配为正式合同调用、映射错误与响应。 | 不得直接持久化。 |
...
| 内部运行任务 | seed 已走 `Operations`；Outbox/提醒/投影是提交后的运行组件。 | 所有会改变业务主数据的任务必须进入正式合同和统一执行器。 | 不得以 scheduler/consumer 名义绕过授权和事务；下游 Outbox 状态和读模型不是业务主数据写入口。 |
| 生成合同 | 当前 12 plans=10 写+2 读；S0-01-03/04/05 已生成创建、WorkItem 写及 UpdateContext/AdvanceGate/Close，S0-01-06/07 已收口 REST/gRPC/MCP。 | 明确写操作的输入、权限、事务与执行合同。 | SaveView、相似度和排期读模型仍为本地扩展，不得误称生成 gRPC 合同。 |
| application Operations | 当前为 adapter、executor 与 service 之间的用例边界；seed、REST、MCP 均由它进入。 | 唯一应用层写入口，负责选择正式 plan 并调用 executor。 | 不得暴露替代执行器的写通道。 |
...
3. 验证文档将 CUR、TGT 与历史 TODO 分开；S0-01-08 仅宣称已验证的 seed 收口，不宣称迁移或切换。
4. 验证 Markdown 内链/源码引用存在，并运行 `git diff --check`。

S0-01-08 已在临时 SQLite/Vault 与本地测试运行中验证 seed、合同生成和投影时序；真实通知、迁移与切换仍不在本任务范围。
