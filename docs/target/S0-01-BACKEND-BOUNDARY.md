...
| CUR-INT seed | 当前事实 / 已验证 | `seedExample` 以 bootstrap 信任边界内直接构造、`Authenticated=true` 的内部 principal（非人类/API key 登录）经 `Operations.List/Create/UpdateContext` 执行；样例写与 Outbox staging 在 SQLite Unit of Work 内提交，dispatcher 再异步投影。 | [bootstrap](../../backend-yunka/internal/bootstrap/application.go)；[seed regression](../../backend-yunka/internal/bootstrap/application_test.go) |
